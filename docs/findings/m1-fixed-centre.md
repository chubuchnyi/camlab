...
Measured 2026-08-10. This is camlab's premise: if it were false, the repo should not exist.

The probe still lives in the pitch3d checkout as `scripts/probe_handheld_centre.py`, because it
drives that repo's `fit_rigid_camera` (544 lines) and its paint masks. camlab has since ported the
measuring half — `camlab.measure.paint` and `camlab.measure.residual` — so the *check* is native
here now; porting the *fit* is M2's job, and M2 needs it anyway.
...
pitch3d's architecture brief of 2026-08-09, §2 concluded that for
...
`camlab` proposal ([the spec](../spec.md)) stands or falls on it.
...
Also settled: **this clip has no 180° roll.** The roll in pitch3d's landmine register is a property of the
