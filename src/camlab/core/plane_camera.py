...
# ---------------------------------------------------------------------------------------------
# Copied from pitch3d 78f94b7 — `src/pitch3d/core/scene/plane_camera.py`.
#
# camlab takes a COPY of the core rather than depending on pitch3d (spec §10.2): the camera
# contract itself has to change here — `CameraTrack` holds ONE intrinsic for a whole clip, so a
# zoom is not representable — and that is exactly what a dependency would forbid. Expect this file
# to diverge; do not hand-sync it back. The transfer format between the two repos is
# `calib/<clip>.npz` (spec §4.3), pinned by tests/test_golden_real_camera.py.
# ---------------------------------------------------------------------------------------------
...
def _orthonormality(h_world_to_image: np.ndarray, k_inv: np.ndarray) -> float:
    """Zhang's residual for one frame.

    How far ``K⁻¹H``'s first two columns are from a rotation's.
    """
    m = k_inv @ h_world_to_image
    r1, r2 = m[:, 0], m[:, 1]
